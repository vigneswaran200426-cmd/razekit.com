// SettlementService — the ONLY place financial state transitions happen after
// provider-authoritative confirmation: payment capture, payout completion and
// refunds. Every transition is idempotent and writes balanced ledger entries.

import { appendLedger } from './ledger.ts';
import { toMajor } from './core.ts';

// ── Payment captured ────────────────────────────────────────────────────────
// Double-entry chain on the brand's wallet:
//   DEPOSIT credit total → PLATFORM_FEE/PROCESSING_FEE/TAX debits (leave the
//   brand wallet) → PRIZE_RESERVATION debit (held for the winner).
// The contest is funded/published and its currency + fee locked.
export async function settleCapturedPayment(sdk, { payment, contest }) {
  ...
  // Contest funding lock: currency + fee snapshot become permanent; publish.
  const contestUpdate = {
    currency,
...
// ── Winner payout completed ─────────────────────────────────────────────────
// Credited to the creator's wallet ONLY on provider-confirmed completion.
...
// ── Refund ──────────────────────────────────────────────────────────────────
// Full refund of an unfunded-outcome contest (cancelled before a winner).
// Refusing to refund after a winner payout is a business rule, not an accident.
