...
import { Compass, Trophy, Award, Bell } from 'lucide-react';
      ...
      <div className="space-y-5">
        <OverviewHeader
          ...
          actions={
            <Link
              ...
              className="relative w-9 h-9 rounded-lg border border-border/70 bg-white/70 flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-primary/30 press transition-colors"
            >
        ...
        <div className="grid lg:grid-cols-12 gap-6 items-start">
              ...
              {loading ? (
                <div className="grid sm:grid-cols-2 gap-4">{[1, 2].map((i) => (
                  <div key={i} className="surface overflow-hidden">
                    ...
                    <div className="p-4 space-y-2.5"><div className="h-4 w-3/4 rounded bg-secondary animate-pulse" /><div className="h-4 w-1/2 rounded bg-secondary animate-pulse" /></div>
                  </div>
                ))}</div>
              ) : active.length ? (
                <div className="grid sm:grid-cols-2 gap-4">{active.slice(0, 4).map((c, i) => <ContestCard key={c.id} contest={c} index={i} />)}</div>
              ) : (
                <EmptyState compact title="No active contests" description="Browse open contests and start your next winning submission." actionLabel="Find contests" to="/explore" />
              )}
            </section>
          ...
          <div className="lg:col-span-4 space-y-6 min-w-0">
            <section><SectionHeader title="Deadlines" /><UpcomingDeadlines contests={myContests} subs={subs} handovers={handovers} winnerPublishes={winnerPublishes} /></section>
            <section><SectionHeader title="Earnings" /><EarningsSummary wallet={wallet} /></section>
            <section><SectionHeader title="Profile" /><ProfileSummary profile={profile} user={user} rating={avgRating} wins={wonContests.length} /></section>
            <section>
              <SectionHeader title="Reputation" />
              <GlassCard className="p-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-primary/10 text-primary flex items-center justify-center"><Award className="w-4 h-4" /></div>
                  <div className="min-w-0"><p className="font-heading text-base font-bold nums">{avgRating ? `${avgRating} rating` : 'No ratings yet'}</p><p className="text-xs text-muted-foreground">{reviews.length} verified review{reviews.length === 1 ? '' : 's'} · {wonContests.length} win{wonContests.length === 1 ? '' : 's'}</p></div>
                </div>
                {reviews.length ? <div className="mt-3 pt-3 border-t border-border/60 space-y-2">{reviews.slice(0, 2).map((r) => <p key={r.id} className="text-xs text-muted-foreground line-clamp-2">“{r.review_text}”</p>)}</div> : <p className="text-xs text-muted-foreground mt-2">Verified brand reviews appear after your first completed contest.</p>}
              </GlassCard>
            </section>
            {socialPosts.length > 0 && <section><SectionHeader title="Social tracker" /><SocialSummaryCard posts={socialPosts} /></section>}
            <section><SectionHeader title="Activity" /><ActivityFeed limit={5} /></section>
          </div>
        </div>
      </div>
    </div>
