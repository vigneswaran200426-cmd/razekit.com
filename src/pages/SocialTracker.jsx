  ...
  return (
    <div className="page-shell pb-10">
      <div className="max-w-6xl mx-auto space-y-5">
        ...
        <section className="surface p-3 md:p-4 space-y-3"><PlatformTabs posts={posts || []} active={platformTab} onChange={setPlatformTab} /><div className="flex flex-col sm:flex-row gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search posts, contests or URLs…" className="pl-9" aria-label="Search social posts" /></div><select className={`${inputCls} sm:w-52`} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} aria-label="Filter by social post status"><option value="all">All statuses</option>{SOCIAL_POST_STATES.map((s) => <option key={s} value={s}>{SOCIAL_POST_STATE_LABELS[s]}</option>)}</select></div></section>
        ...
        <AddPostDialog open={addOpen} onClose={() => setAddOpen(false)} contests={contests} winnerPublishes={winnerPublishes} onCreated={load} /><PublishDialog key={publishPost?.id || 'none'} post={publishPost} onClose={() => setPublishPost(null)} onSaved={load} /><MetricsDialog post={metricsPost} onClose={() => setMetricsPost(null)} onSaved={load} />
      </div>
    </div>
  );
}
